...
# ═══════════════════════════════════════════════════════════════
#  DYNAMIC WATCHLIST — fetches live from NSE official index CSVs
#  Covers Nifty500 + Midcap150 + Smallcap250 + Microcap250
#  Falls back to hardcoded 600-stock list if NSE is unreachable.
#  Min market cap filtering happens at scan time via the sidebar
#  slider — so all 1000+ tickers are fetched but only those
#  above your chosen threshold are actually scanned.
# ═══════════════════════════════════════════════════════════════
...
NSE_INDEX_URLS = [
    ...
    "https://archives.nseindia.com/content/indices/ind_nifty_next_50_list.csv",
]
...
def get_live_watchlist():
    """
    Fetches NSE constituent lists and returns all unique .NS tickers.
    Cached for 24 hours. Falls back to hardcoded list if NSE is unreachable.
    """
    ...
    # NSE unreachable — use hardcoded fallback
    return list(FALLBACK_WATCHLIST)


# ── FALLBACK — used only if NSE URLs are all unreachable ──────────────
FALLBACK_WATCHLIST = [
    ...
    "HDFCAMC.NS","NIPPONLIFE.NS","HDFCLIFE.NS","SBICARD.NS","ICICIGI.NS",
    "STARHEALTH.NS","NUVAMA.NS","PFC.NS","BANKBARODA.NS","RVNL.NS",
    "PVRINOX.NS","KALYANKJIL.NS","GRINDWELL.NS","DHANUKA.NS","MTARTECH.NS",
    "PGHL.NS","ABSLAMC.NS","CDSL.NS","MANYAVAR.NS","HONASA.NS",
    "POLICYBZR.NS","PAYTM.NS","NYKAA.NS","ZOMATO.NS","JIOFIN.NS",
]
